...
class ActorCriticAgent:
    ...
    def run_episode(self, initial_state, max_steps=1000):
        ...
        @return: returns log probabilities, values, rewards in each step of the episode
        """
    ...
    def compute_loss(self, action_log_probs, values, returns):
        """ Computes the combined actor-critic loss.

        @param action_log_probs: log probability of action taken at each step
        @param values:  value of the state at each step
        @param returns: discounted cumulative rewards at each step
        @return: sum of actor and critic loss
        """
    ...
    def train_step(self, initial_state, max_steps_per_episode=1000):
        ...
        with tf.GradientTape() as tape:
            action_log_probs, values, rewards = self.run_episode(initial_state, max_steps_per_episode)

            returns = self.get_expected_return(rewards)

            values, returns = [tf.expand_dims(x, 1) for x in [values, returns]]

            loss = self.compute_loss(action_log_probs, values, returns)
